...
public class Main {
	...
	input.addActionListener(new ActionListener() {
		...
		if (input.getText().startsWith("/")) {
			...
			if (Player.alive) {
			    Main.sendMessage("[!] You are already alive.");
			} else {
				...
				public void run() {
					...
					Network.sendSecure(respawnBatch);
					this.cancel();
				    } else {
					Main.sendMessage("[!] Respawning in " + i + "...");
				    }
				    i--;
				}
			    }, 0, 1000);
			...
			Network.sendSecure(new Batch("message").add("uuid", Player.uuid).add("message", input.getText()));
		    }
		}
		input.setText("");
	    }
	...
	thread = new Thread(Main::run, "main");
	...
	sendMessage(
		"   Welcome to Electro, A simple game written from scratch in Java over 4 days for Electro-Tech 11.\n\nHave Fun!\n   ~ Jeremy");
