package com.jeremy.electro.input;
...
public class Input {
	...
	Main.canvas.addMouseListener(new MouseAdapter() {
		...
		PRESSED_MOUSE.add(event.getButton());
	    }
	...
	Main.canvas.addMouseMotionListener(new MouseMotionListener() {
	...
	Main.canvas.addMouseWheelListener(new MouseWheelListener() {
	...
	Main.canvas.addKeyListener(new KeyAdapter() {
		...
		TYPE_CALLBACKS.forEach(callback -> callback.keyTyped(event));
	    }
	...
	Main.canvas.addFocusListener(new FocusAdapter() {
    ...
    public static void addKeyTypedCallback(TypeCallback callback) {
	TYPE_CALLBACKS.add(callback);
    }
