...
public class Receiver {
	...
	public static void receive(String address, Bundle batch) {
		try {
			...
			if (type.equals("connect")) {
				...
				String udpAddress = address.split(":")[0] + ":" + udpPort;
				Client client = new Client(name, uuid, address, udpAddress);
				Network.CONNECTED_PLAYERS.put(address, client);
			...
			if (type.equals("connect")) {
				...
				Bundle soundBatch = new Bundle("sound");
				soundBatch.put("name", batch.get("name"));
				soundBatch.put("name", batch.get("name"));
